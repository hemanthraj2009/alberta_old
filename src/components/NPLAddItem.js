...
export default class NPLAddItem extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({ tax1: tax1 })
    const tax2 = this.props.navigation.getParam('tax2')

    this.setState({ tax2: tax2 })
    ...
    AsyncStorage.getItem('token').then((data) => {
      AsyncStorage.getItem('Sid').then((SID) => {
        if (data) {
          ...
          return fetch(`https://portal.albertapayments.com/api/admin/department/list/${SID}?token=${encodeURIComponent(data)}`)
  ...
  getCategoryList = (deparmentID, isLoad = 0) => {
    ...
    AsyncStorage.getItem('token').then((data) => {
      AsyncStorage.getItem('Sid').then((SID) => {
        if (data) {
          return fetch(`https://portal.albertapayments.com/api/admin/category/list/${SID}/${deparmentID}?token=${encodeURIComponent(data)}`)
            .then((response) => response.json())
            .then((responseJson) => {
    ...
    AsyncStorage.getItem('token').then((data) => {
      AsyncStorage.getItem('Sid').then((SID) => {
        if (data) {
          ...
          return fetch(`https://portal.albertapayments.com/api/admin/subcategorysid?category_id=${subCategoryID}&sid=${SID}&token=${encodeURIComponent(data)}`)
  ...
  saveNPLItemDetails = (deparmentID) => {
    ...
    AsyncStorage.getItem('token').then((data) => {
      ...
      AsyncStorage.getItem("Sid").then(sid => {
        if (sid) {
          if (data) {
            fetch(`https://portal.albertapayments.com/api/admin/insert/item/customer?token=${encodeURIComponent(data)}`, {
              method: 'POST',
              headers: {
        ...
        <KeyboardAwareScrollView>
          ...
          <ScrollView>
            ...
            <View
              ...
              >
              <Text style={styles.setTextSize}>Sales</Text>
              </View>
              <View
                style={{
                  width: "60%"
...
{showcategary ? <View style={styles.logocontainer}>
 ...
 : <View></View>}
            ...
            </View><View style={styles.foodContainer}>
            <View
                style={{
                  width: "40%"
                }}
