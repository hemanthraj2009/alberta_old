...
import {
    ...
    TextInput, AsyncStorage, Dimensions, Vibration, ScrollView, Alert
} from 'react-native';
...
export default class ChooseItem extends Component {
    ...
    componentDidMount() {
        ...
        // this.state.ipoid = AsyncStorage.getItem('ipoid');
        ...
        AsyncStorage.getItem('token').then((data) => {
            AsyncStorage.getItem('Sid').then((SID) => {
                if (data) {
                    ...
                    fetch(API_BASE_URL + `admin/get_itemdetails_with_barcode?sid=${SID}&barcode=${this.state.barcode}&token=${data}`, {
                        method: 'GET',
                        ...
                        .then((responseJson) => {
                            ...
                            if (responseJson.status == "success") {
                                ...
                                var costPrice = JSON.stringify(responseJson.item_data[0].new_costprice);
                                ...
                                var new_costprice = JSON.stringify(responseJson.item_data[0].new_costprice);
    ...
    vendorItemCodeMethod = () => {
        ...
        if (this.state.vendorItemCode == "") {
            Alert.alert(
                ...
                '',
                'Vendor code missing Please check',
                [
                    { text: 'OK', },
                ]
            )
        ...
        AsyncStorage.getItem('token').then((data) => {
            AsyncStorage.getItem('Sid').then((SID) => {
                if (data) {
                    ...
                    fetch(API_BASE_URL + `admin/get_item_with_vendoritemcode?sid=${SID}&vendoritemcode=${this.state.vendorItemCode}&token=${data}`, {
                        method: 'GET',
    ...
    itemNameSearch = () => {
        ...
        fetch(API_BASE_URL + `get_item_with_name?sid=1001&keyword=${this.state.searchItemName}`, {
            method: 'GET',
        ...
        return (
            ...
            <View style={styles.container}>
                ...
                <View style={{ marginTop: 5, alignContent: 'center', alignItems: 'center' }}>
                    <Text style={{ fontSize: 25, fontWeight: '700', color: '#696969' }}>Choose Item</Text>
                </View>
